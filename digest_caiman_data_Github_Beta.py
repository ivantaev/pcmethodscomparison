...
from PF_analysis_CL_VI_Github_Beta_Sandbox import (main) #main_multiday_V2
...
for m in range(len(Mouse)):
    ...
    path = [r'C:\Users\Vlad\Desktop\BCF\Alis_data\Data\Mouse%d\Day%d' %(Mouse[m], days[m][j]) for j in range(len(days[m]))]
    ...
    for j in range(len(datatype)):
        names = [dirct + r'\comparison\Data_Mouse%d_new_multiday.hdf5' %Mouse[m], trajname]
        savenames = [[dirct + r'\comparison\data_outputs\Mouse%d_day%d_%s_%dshuff_adaptedshuffles_circshuffles_%s.mat' 
